...
// Professor Solution to Task 1
  ...
  public int RemoveEnd()
  {
    ...
    return tdata; // return data stored at temp data from last node data, now nullyfied
  }
  ...
  public void AddPosition(int n, int pos)
  {
    ...
    for(int i = 0; i < pos; i++, temp = temp.next); // run thru array and moving temp pointer along with counter
...
class REFCL01
{
  static void Main(string[] args)
  {
    ...
    Console.Write($"{testa.data} <- Teste A\n{testb.data} <- Teste B\n");
  }
}